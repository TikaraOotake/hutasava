...
[CreateAssetMenu(fileName = "Hammer", menuName = "ScriptableObjects/CreateWeapon_Hammer")]
public class Hammer : Weapon
{
    [SerializeField] private float AttackLength;//攻撃距離
    ...
    public override void Update_Item(GameObject _CallObject)//引き数：_CallObject　呼び出し元のオブジェクト
    {
        ...
        if (AttackCooltimer == 0.0f)
        {
            if (BulletPrefab != null)
            {
                ...
                {
                    //方向を算出
                    direction = (Enemy.transform.position - _CallObject.transform.position).normalized;
                ...
                GameObject newBullet = Instantiate(BulletPrefab, _CallObject.transform.position + direction * AttackLength, Quaternion.identity);
